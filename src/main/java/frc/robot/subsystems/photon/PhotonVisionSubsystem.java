...
public class PhotonVisionSubsystem extends SubsystemBase {
  ...
  // Constructor initializes the PhotonVision subsystem. It sets up AprilTag field layout and pose estimators for the cameras. @param swervePoseEstimator The robot's pose estimator for integrating vision data.
  public PhotonVisionSubsystem(SwerveDrivePoseEstimator swervePoseEstimator) {
    ...
    poseEstimator1 =
        new PhotonPoseEstimator(
            fieldLayout, 
            PoseStrategy.MULTI_TAG_PNP_ON_COPROCESSOR,  // Strategy that selects the pose with the least uncertainty for multi-tag detection
  ...
  // Called periodically by the robot framework. This method updates the robot's position using vision data, and enables possibility to add filtering.
  @Override
  public void periodic() {
    updateOdometry();
  }

  // Updates the robot's pose estimation by processing AprilTag detections from both cameras. If both cameras detect tags, it averages their poses to improve accuracy. The pose estimation is then added to the swerve pose estimator.
  ...
  public void updateOdometry() {
    ...
    if (estimate1.isPresent() && estimate2.isPresent()) {
        ...
        Pose3d combinedPose = new Pose3d(
        ...
        swervePoseEstimator.addVisionMeasurement(combinedPose.toPose2d(), timestamp);  // Convert back to Pose2d
